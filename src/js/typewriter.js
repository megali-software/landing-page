...
function typeWriter() {
  ...
  if (isDeleting) {
    document.getElementById("typewriter").innerHTML = displayText.substring(0, i) + '<span class="blinking-cursor">|</span>';
    if (i === 0) {
      isDeleting = false;
      currentText = (currentText + 1) % txt.length;
      setTimeout(typeWriter, speed + Math.floor(Math.random() * randomSpeedVariation));
    } else {
      i--;
      setTimeout(typeWriter, speed + Math.floor(Math.random() * randomSpeedVariation));
    }
